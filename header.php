					...
					<div id="navbar-1" class="navbar-collapse collapse">
						<div class="col-sm-12">
							<ul class="nav navbar-nav navbar-right">
								<li><a href="<?= esc_url( home_url( '/get-help/' ) ); ?>">Get Help</a></li>
								<?php $blog_page = get_option('page_for_posts'); ?>

								<!-- blog page link -->
								<li>
									<a href="<?= $blog_page ? get_permalink( $blog_page ) : bloginfo('url'); ?>" <?= is_blog() ? 'class="custom-active"' : ''; ?>>
										<?= $blog_page ? get_page($blog_page)->post_title : __('Blog', 'fractal'); ?>
									</a>
								</li>
								...
								<?php if ( is_plugin_active( 'polylang/polylang.php' && $polylang ) ): ?>
									<li class="hidden-xs dropdown">
										<?php $switcher = pll_the_languages(array('raw'=>1)); ?>
											<?php $lang = array(); ?>
											<?php for ($i=0; $i<count($switcher); $i++): ?>
												<?php $lang[]=$switcher[$i]['classes']; ?>
												<?php if (in_array('current-lang', $lang[$i])): ?>
													<a href="#" class="dropdown-toggle" data-toggle="dropdown">
														<img src="<?= $switcher[$i]['flag']; ?>"/>
														<span><?= $switcher[$i]['name']; ?></span><b class="caret"></b>
													</a>
												<?php endif; ?>
											<?php endfor; ?>
										<ul class="dropdown-menu">
											<?php for ($i=0; $i<count($switcher); $i++): ?>
												<li>
													<a href="<?= $switcher[$i]['url']; ?>" <?php if (in_array('current-lang', $lang[$i])) echo('class="custom-active"'); ?>>
														<img src="<?= $switcher[$i]['flag']; ?>"/> <?= $switcher[$i]['name']; ?>
													</a>
												</li>
											<?php endfor; ?>
										</ul>
									</li>
								<?php endif; ?> <!-- End of language switcher -->
